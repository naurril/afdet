...
#scene_name = "2020-06-28-14-21-56"
#scene_name = "sustechscapes-mini-dataset"
scene_name = "kitti"
...
def downsample_points(points):
    ...
    d = 20
    grids = np.round(points[:,0:3] * d)
    points = np.concatenate([grids, points], axis=-1)

    df = pd.DataFrame(points)
    d = df.groupby([0,1,2]).mean()
    return d.to_numpy()

def build_pillar_image(d,f,save_path):    
    "dataset, frame, savepath"
    print(f)
    points = d.load_point_cloud(scene_name,f)

    points = points[(points[:,2]>=-3) & (points[:,2] <= 1) & (points[:,0] <= 40) & (points[:,0]>= -40) & (points[:,1]<=40) & (points[:,1] >= -40)]
    points = downsample_points(points)
    points = points.astype(np.float32)
    #points.tofile(f"/home/lie/fast/code/SUSTechPoints-be/data/{scene_name}/lidar.bin/{f}-1.bin")

    pillars,coord = cloud_to_pillars(points)
...
def build_gt_file(d,f,save_path):
    ...
    label = d.load_label(scene_name, f)
...
def prepare_raw_data(func):
    ...
    save_path = f'./data/{scene_name}-afdet'

    d = SustechScapesDataset(sustechscapes_root_dir, [scene_name])
    scene = d.get_scene(scene_name)
